...
//! - File store (`~/.larc/secrets.toml`, chmod 600) via [`FileStore`]
...
///
/// Default path: `~/.larc/secrets.toml`.
...
impl FileStore {
    /// Create a `FileStore` using the default path `~/.larc/secrets.toml`.
    ///
    /// # Errors
    ///
    ...
    pub fn new() -> Result<Self> {
        ...
        let path = PathBuf::from(home).join(".larc").join("secrets.toml");
        Ok(Self { path })
    }
...
///
/// ```
/// use l_arc_crypto::secrets::{resolve_secret, FileStore, SecretStore};
///
...
///
/// ```
/// use l_arc_crypto::secrets::{auto_generate_and_persist, FileStore, SecretStore};
...
mod tests {
    ...
    fn unique_env_store() -> EnvStore {
        let id = crate::random::generate_hex(4);
