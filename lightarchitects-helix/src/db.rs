...
use crate::graph::neo4j::Neo4jBackend;
...
use crate::search::{ScoredResult, SearchOptions, index_names};
use crate::types::{
...
pub trait HelixDb: Send + Sync {
    ...
    async fn ensure_helix(
        ...
        owner: &str,
        name: &str,
        ordering_mode: crate::types::HelixOrderingMode,
    ) -> Result<String, HelixDbError>;
    ...
    async fn register_source(
        &self,
        source: &crate::types::SourceWatermark,
    ) -> Result<String, HelixDbError>;
    ...
    async fn write_personality(
        &self,
        helix_id: &str,
        profile: &crate::types::PersonalityProfile,
    ) -> Result<(), HelixDbError>;
    ...
    async fn batch_assign_strand(
        ...
        for step_id in step_ids {
            self.assign_to_strand(&crate::types::StrandMembership {
    ...
    -> Result<Vec<crate::graph::Record>, HelixDbError>;
    ...
    async fn health(&self) -> Result<crate::graph::HealthStatus, HelixDbError>;
}
...
impl HelixConfig {
    ...
    /// Build a [`crate::cache::HelixCacheConfig`] from this config.
    #[must_use]
    pub fn cache_config(&self) -> crate::cache::HelixCacheConfig {
        crate::cache::HelixCacheConfig::default()
...
impl HelixNeo4j {
    ...
    async fn timed_execute(
        ...
        crate::training::record_cypher_call(operation, cypher, duration_ms);
    ...
    /// is fixed at [`crate::graph::DEFAULT_POOL_SIZE`].
    ///
    ...
    pub fn pool_metrics(&self) -> PoolMetrics {
        PoolMetrics {
            max_connections: crate::graph::DEFAULT_POOL_SIZE,
...
impl HelixDb for HelixNeo4j {
    ...
    async fn migrate(&self) -> Result<u32, HelixDbError> {
        ...
        crate::migrations::validate_helix_migrations()
        ...
        let pending = crate::migrations::helix_pending_migrations(&applied_versions);
        ...
        for migration in pending {
            ...
            for stmt in migration.statements {
                ...
                self.backend.execute(stmt, params).await.map_err(|e| {
                    HelixDbError::Graph(crate::graph::GraphError::Schema(format!(
...
impl HelixNeo4j {
    ...
    fn records_to_scored_steps(records: &[crate::graph::Record]) -> Vec<ScoredResult<Step>> {
        records
            .iter()
            .filter_map(|record| {
    ...
    fn record_to_step(record: &crate::graph::Record) -> Option<Step> {
    ...
    fn record_to_shared_experience(record: &crate::graph::Record) -> Option<SharedExperience> {
        let id = record.get("id")?.as_str()?.to_owned();
        let weight = record
            .get("weight")
            .and_then(serde_json::Value::as_f64)
...
mod tests {
    ...
    fn test_record_to_step_minimal() {
        ...
        let record = crate::graph::Record { fields };
