...
///
/// Produces reproducible vectors by hashing the input text.
...
impl MockEmbeddingProvider {
    ...
    /// Create a mock provider matching structural embeddings (128 dims).
    #[must_use]
    ...
    fn hash_to_vector(&self, text: &str) -> Vec<f32> {
        // Use a simple hash-based PRNG for reproducibility.
        // FNV-1a-inspired hash seeded from text bytes.
...
mod tests {
    ...
    async fn test_deterministic_embeddings() {
        ...
        let v1 = provider.embed(&["hello world"]).await.unwrap_or_default();
        let v2 = provider.embed(&["hello world"]).await.unwrap_or_default();
    ...
    async fn test_different_inputs_differ() {
        let provider = MockEmbeddingProvider::nomic();
        let v1 = provider.embed(&["hello"]).await.unwrap_or_default();
        let v2 = provider.embed(&["world"]).await.unwrap_or_default();
    ...
    async fn test_correct_dimensions() {
        let provider = MockEmbeddingProvider::nomic();
        let results = provider.embed(&["test"]).await.unwrap_or_default();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].len(), 768);
    }

    #[tokio::test]
    async fn test_structural_dimensions() {
    ...
    async fn test_batch_embedding() {
        ...
        let results = provider.embed(&texts).await.unwrap_or_default();
    ...
    async fn test_empty_batch() {
        let provider = MockEmbeddingProvider::nomic();
        let results = provider.embed(&[]).await.unwrap_or_default();
    ...
    async fn test_l2_normalized() {
        let provider = MockEmbeddingProvider::nomic();
        let results = provider.embed(&["normalize me"]).await.unwrap_or_default();
