...
/// A single event emitted during streaming LLM generation.
///
/// Maps directly to the Anthropic Messages API SSE event shape (TS-1 §B.1).
/// Providers that do not natively stream emit events synthesised from their
/// batch [`AgentResponse`] via the default [`LlmAgentProvider::spawn_streaming`]
/// implementation.
#[derive(Debug, Clone)]
pub enum ProviderEvent {
    /// Generation started; carries model identity and prompt token count.
    MessageStart {
        /// Provider-reported model name (e.g. `"claude-sonnet-4-6"`).
        model: String,
        /// Input tokens consumed by this request.
        input_tokens: u32,
    },
    /// A content block (text or `tool_use`) opened at `index`.
    ContentBlockStart {
        /// Position of this block within the message's `content` array.
        index: u32,
        /// `"text"` or `"tool_use"`.
        block_type: String,
        /// Tool call identifier — present only when `block_type == "tool_use"`.
        tool_use_id: Option<String>,
        /// Tool name — present only when `block_type == "tool_use"`.
        tool_name: Option<String>,
    },
    /// Incremental text fragment within a `text` content block.
    TextDelta {
        /// Index of the owning content block.
        index: u32,
        /// The incremental text.
        text: String,
    },
    /// Incremental JSON fragment within a `tool_use` content block.
    InputJsonDelta {
        /// Index of the owning content block.
        index: u32,
        /// Partial JSON string; callers must accumulate before parsing.
        partial_json: String,
    },
    ...
    /// The message is complete; carries stop reason and output token count.
    MessageDelta {
    ...
    /// Final event — stream is exhausted.
    MessageStop,
    /// A tool result being fed back into the LLM context after execution.
    ToolResult {
...
pub trait LlmAgentProvider: Send + Sync {
    ...
    /// Stream the agent response as a sequence of [`ProviderEvent`]s (TS-1 §B.1).
    ///
    /// Providers that natively support streaming MUST override this method.
    /// The default implementation calls [`spawn`] and synthesises a minimal
    /// event sequence from the batch [`AgentResponse`], making every provider
    /// compatible with streaming call sites without code changes.
    ///
    /// The returned stream is `'static` — it does not borrow from `self`.
    ///
    /// # Errors
    ///
    /// Returns [`ProviderError`] if the underlying spawn fails.
    ///
    /// [`spawn`]: LlmAgentProvider::spawn
    async fn spawn_streaming(
        &self,
        req: SanitizedAgentRequest,
    ) -> Result<BoxStream<'static, ProviderEvent>, ProviderError> {
        let response = self.spawn(req).await?;
        let text = response.output.to_string();
        let events = vec![
            ProviderEvent::MessageStart {
                model: "batch-provider".to_owned(),
            ...
            ProviderEvent::TextDelta { index: 0, text },
