...
use crate::crypto::verses::{Verse, random_verse, verse_hkdf_info_with_purpose};
...
pub fn derive_api_key(pepper: &SecretString, env: &str, verse: &Verse) -> Result<DerivedKey> {
    let ikm = crate::crypto::random::generate_bytes(32);
    ...
    let hash = crate::crypto::hash::hmac_hash(pepper, body_with_checksum.as_bytes())?;
