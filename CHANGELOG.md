...
- Renamed workspace `la-sdk` → `l-arc-sdk`, `la-crypto` → `l-arc-crypto`
...
#### Phase 3 — `l-arc-core` Foundation
- `l-arc-crypto` — scripture-forged cryptographic foundation
...
- `l-arc-core` — MCP wire protocol foundation
...
- `l-arc-soul` — SOUL typed client (`soulTools`, 23 actions, fluent `helix` and `query` builders)
- `l-arc-corso` — CORSO typed client (`corsoTools`, 26 actions, structured response types)
- `l-arc-eva` — EVA typed client (`evaTools`, 8 actions, dual-path: typed methods + generic adapter)
- `l-arc-quantum` — QUANTUM typed client (`qsTools`, 13 actions, `mcp-server` subcommand handled automatically)
- `l-arc-seraph` — SERAPH typed client (`penTools`, 18 actions, `Content-Length` framing)

#### Phase 9-11 — Umbrella, Observability, and CLI
- `l-arc-ayin` — feature-gated AYIN observability wrapper (`observe` feature = compile-time opt-in)
  - Zero-cost newtype when feature is disabled; fire-and-forget span I/O when enabled
- `l-arc` — umbrella crate with feature-gated sibling re-exports (`soul`, `corso`, `eva`, `quantum`, `seraph`, `ayin`, `full`)
  - `tracing-fmt` feature for CLI/app tracing initialisation
- `l-arc-cli` — `larc` binary with `ping`, `health`, `version` subcommands; rich table output via `l-arc-arena`

#### Phase 12 — Integration Tests
- `l-arc-core/tests/transport_integration.rs` — end-to-end JSON-RPC round-trip tests (7 scenarios)
- `l-arc-core/tests/retry_integration.rs` — transient error retry verification
- `l-arc-core/tests/sibling_ids_integration.rs` — binary path and framing validation
- All tests run without spawning real sibling binaries (mock transport)

#### Phase 13 — Observability Integration
- `l-arc-ayin` wired into `l-arc-core` transport via optional `observe` feature
...
- `cargo audit` clean — one accepted WARNING (RUSTSEC-2025-0119: `number_prefix` unmaintained, l-arc-arena only)
...
- `l-arc-core/src/transport.rs` — 12 adversarial unit tests covering all framing failure modes:
...
- `l-arc-core/tests/adversarial.rs` — integration-level adversarial tests:
...
- All per-crate READMEs written (`l-arc-soul`, `l-arc-corso`, `l-arc-eva`, `l-arc-quantum`, `l-arc-seraph`, `l-arc-ayin`, `l-arc`)
- `cargo doc --workspace --no-deps` produces zero warnings
- `cargo test --doc` passes (all doc examples verified)
- Resolved intra-doc link issues across `l-arc-ayin`, `l-arc-seraph`, `l-arc-quantum`, `l-arc-eva`, `l-arc-crypto`, `l-arc-arena`, `l-arc-auth`
- Binary target `doc = false` on `l-arc-cli` to prevent filename collision with umbrella crate

### Changed

- `l-arc-arena/src/discovery.rs` — resolved `ToolRegistry` intra-doc link
- `l-arc-cli/Cargo.toml` — `doc = false` on binary target to prevent doc filename collision

### Fixed

- `l-arc-core` content-length framing: header count now bounded to 32 (prevents unbounded memory allocation from malicious sibling responses)
