...
//!
//! For Neo4j-backed operations, use [`lightarchitects_helix::HelixStore`].
//!
//! # Examples
//!
//! ```rust,no_run
//! # async fn example() -> Result<(), lightarchitects_soul::SoulError> {
...
use crate::pipeline::{RetrievalHit, RetrievalPipeline};
...
/// operations, use [`lightarchitects_helix::HelixStore`] instead.
pub struct SoulDb {
...
impl SoulDb {
    ...
    /// [`FastEmbed`][crate::embedding::fastembed::FastEmbedProvider] ONNX model.
    ///
    ...
    pub fn with_fastembed(self) -> Result<Self, SoulError> {
        use crate::embedding::fastembed::{FastEmbedModel, FastEmbedProvider};
        let provider = FastEmbedProvider::try_new(FastEmbedModel::Default).map_err(|e| {
            SoulError::Storage(crate::storage::StorageError::InvalidArgument(format!(
                "FastEmbed init: {e}"
            )))
        })?;
    ...
    /// [`FastEmbedModel`][crate::embedding::fastembed::FastEmbedModel].
    ///
    /// Requires the `embedding-fastembed` feature.
    ///
    /// # Errors
    ...
    pub fn with_fastembed_model(
        self,
        model: crate::embedding::fastembed::FastEmbedModel,
    ...
    pub fn with_ollama(self, base_url: impl Into<String>) -> Result<Self, SoulError> {
        use crate::embedding::ollama::{OllamaConfig, OllamaEmbeddingProvider};
    ...
    pub fn with_llama_cpp(self, base_url: impl Into<String>) -> Result<Self, SoulError> {
        use crate::embedding::llama_cpp::LlamaCppEmbeddingProvider;
    ...
    pub async fn ingest_directory(&self, dir: &Path) -> Result<usize, SoulError> {
        use crate::ingestion::vault::load_directory;
    ...
    /// ```rust,no_run
    /// # async fn example() -> Result<(), lightarchitects_soul::SoulError> {
    /// # let soul = lightarchitects_soul::SoulDb::memory()?;
