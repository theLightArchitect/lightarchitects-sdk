...
//! # async fn example(client: lightarchitects_soul::SoulClient<lightarchitects_core::StdioTransport>)
//! # -> Result<(), lightarchitects_core::SdkError> {
//! use lightarchitects_soul::{IngestSource, TextFormat};
//!
//! let result = client
//!     .graphrag_ingest()
...
use crate::types::GraphRagIngestResult;
...
/// Fluent builder for the `soulTools` `graphrag_ingest` action.
///
/// Created via [`crate::SoulClient::graphrag_ingest`]. Chain optional
...
mod tests {
    ...
    async fn builder_without_source_returns_error() {
        use lightarchitects_core::{McpClient, MockTransport, RetryConfig, SdkError};
