...
//! [`SQLite`][crate::sqlite::SqliteBackend], Neo4j, and dual-write backends.
...
//!
//! [`HelixEntry`][crate::HelixEntry] is the **MCP client response type** — what
...
//!
//! ```rust,no_run
//! use lightarchitects_soul::storage::{EntryFilter, StorageBackend};
//! # use std::sync::Arc;
//! # async fn example<B: StorageBackend>(backend: Arc<B>) -> Result<(), lightarchitects_soul::storage::StorageError> {
...
/// with [`crate::HelixEntry`], the MCP client response type returned by
...
/// shadowing [`crate::SearchHit`], the MCP client response type.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn default_storage_path() -> PathBuf {
    lightarchitects_core::paths::soul_or_fallback()
}
