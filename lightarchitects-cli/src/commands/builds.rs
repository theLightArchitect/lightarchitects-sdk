...
pub enum BuildsCommand {
    ...
    Show {
        /// Build name (case-insensitive), e.g. "corso", "webshell".
        name: String,
    },
}
...
/// # Errors
///
/// - [`SdkError::Config`] if the vault is not configured or `active.yaml` is missing.
/// - [`SdkError::Config`] if `active.yaml` cannot be parsed.
pub fn execute(cmd: BuildsCommand, mode: OutputMode) -> Result<(), SdkError> {
    let doc = load_builds()?;
...
fn load_builds() -> Result<BuildsDocument, SdkError> {
    let Some(helix_root) = lightarchitects_core::paths::helix_root() else {
        return Err(SdkError::Config(
            "SOUL vault not configured — set LIGHTARCHITECTS_HOME or create ~/.soul/helix/"
                .to_owned(),
        ));
    };
    ...
    let content = std::fs::read_to_string(&path)
        .map_err(|e| SdkError::Config(format!("failed to read {}: {e}", path.display())))?;

    serde_yaml::from_str(&content)
        .map_err(|e| SdkError::Config(format!("failed to parse {}: {e}", path.display())))
}
...
fn show_build(doc: &BuildsDocument, name: &str, mode: OutputMode) -> Result<(), SdkError> {
    let build = doc
        .builds
        .iter()
