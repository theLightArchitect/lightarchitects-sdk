...
| `lightarchitects-gateway` | Binary (`lightshell`) | MCP server + Arena HTTP + Conductor |
| `lightarchitects-webshell` | Binary (`lightspace`) | Local web GUI (PTY + 3D helix panel) |
...
make deploy         # quality + build + deploy gateway → ~/.lightarchitects/bin/lightshell
...
```
~/.lightarchitects/bin/lightshell         ← binary
~/.lightarchitects/bin/lightshell.prev    ← previous binary (auto-saved by make deploy)
~/.lightarchitects/lightshell.mcp.json    ← MCP config (auto-generated by make deploy)
...
# Binary lands at: lightarchitects-gateway/target/release/lightshell
...
cp lightarchitects-gateway/target/release/lightshell "$GATEWAY_BIN"
...
printf '{\n  "mcpServers": {\n    "lightshell": {\n      "command": "%s"\n    }\n  }\n}\n' \
    "$GATEWAY_BIN" > "$HOME/.lightarchitects/lightshell.mcp.json"
```
...
`make deploy` handles codesigning automatically. If copying the binary manually (`cp target/release/lightshell ~/.lightarchitects/bin/`), the binary will be SIGKILL'd (exit 137) on first run by Gatekeeper unless signed:

```bash
codesign -s - ~/.lightarchitects/bin/lightshell
```
