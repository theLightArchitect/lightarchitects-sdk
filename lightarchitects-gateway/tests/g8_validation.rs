...
fn build_state_with_graph(
    ...
    let helix_limiter = Arc::new(governor::RateLimiter::keyed(governor::Quota::per_minute(
...
async fn g8_08_health_returns_200_healthy() {
    ...
    let resp = app
        .oneshot(get("/v1/platform/health", TEST_IP))
        .await
        .unwrap();
    ...
    assert_eq!(
        body["status"], "healthy",
        "G8-8: status field must be 'healthy'"
    );
}
...
async fn g8_09_unauthenticated_read_returns_401() {
    ...
    let app = build_http_router(state);
    let resp = app
        .oneshot(get("/v1/platform/canon/anything", TEST_IP))
        .await
        .unwrap();
    assert_eq!(
        resp.status(),
        StatusCode::UNAUTHORIZED,
        "G8-9: missing token must be 401, not {:?}",
        resp.status()
    );
}
...
async fn g8_10_read_bearer_on_admin_returns_403() {
    ...
    let app = build_http_router(state);
    let body =
        serde_json::json!({"path":"test","kind":"canon","content_text":"x","version":"1.0.0"});
    let resp = app
        .oneshot(post_bearer(
    ...
    assert_eq!(
        ...
        "G8-10: read bearer on admin must be 403"
    );
    let json = body_json(resp).await;
    assert_eq!(
        json["error"]["code"], "insufficient_scope",
        "G8-10: error code must be insufficient_scope"
    );
}
...
async fn g8_11_rate_limit_returns_429_with_retry_after() {
    ...
    assert_eq!(
        ...
        "G8-11: 4th write req must be 429 (limit=3)"
    );
}
...
async fn g8_13_integration_cache_control_on_canon() {
    ...
    let cc = resp
        ...
        .unwrap_or("");
    assert!(
        cc.contains("max-age=2592000"),
        "G8-13: Cache-Control must contain max-age=2592000, got: {cc}"
    );
}
...
async fn g8_14_version_header_present_on_all_responses() {
    ...
    let resp404 = app
        .oneshot(get("/v1/platform/nonexistent-route-xyz", TEST_IP))
        .await
        .unwrap();
    assert!(
...
async fn g8_01_integration_canon_get_200_with_content_hash() {
    ...
    let body = body_json(resp).await;
    assert!(
        body.get("content_hash").is_some(),
        "G8-1: response must include content_hash"
    );
...
async fn g8_02_integration_etag_304_on_repeat() {
    ...
    assert_eq!(
        ...
        "G8-2: repeat with If-None-Match must be 304"
    );
...
async fn g8_03_integration_org_override_composite() {
    ...
    assert!(
        base.get("org_label").is_none(),
        "G8-3: base must NOT have org_label"
    );
    ...
    assert_eq!(
        resp_org.status(),
        StatusCode::OK,
        "G8-3: org override request must be 200"
    );
...
async fn g8_04_integration_locked_field_violation() {
    ...
    assert_eq!(
        ...
        "G8-4: locked entry upload must be 400"
    );
    let json = body_json(resp).await;
    assert_eq!(
...
async fn g8_05_integration_admin_upload_201() {
    ...
    assert_eq!(
        resp.status(),
        StatusCode::CREATED,
        "G8-5: admin upload must return 201 Created"
    );
    let json = body_json(resp).await;
    assert_eq!(
        json["path"], "g8-upload-test",
        "G8-5: response must echo the path"
    );
    assert!(
        json.get("content_hash").is_some(),
        "G8-5: response must include content_hash"
    );
}
...
async fn g8_06_admin_upload_with_read_bearer_returns_403() {
    ...
    let body =
        serde_json::json!({"path":"g8-test","kind":"canon","content_text":"x","version":"1.0.0"});
    let resp = app
        .oneshot(post_bearer(
            "/v1/admin/canon/upload",
            body,
    ...
    assert_eq!(
        resp.status(),
        StatusCode::FORBIDDEN,
        "G8-6: read bearer on admin must be 403"
    );
}
...
async fn g8_07_integration_vault_info_schema() {
    let state = build_neo4j_state(None).await;
    let app = build_http_router(state);
    let resp = app.oneshot(get("/v1/vault/info", TEST_IP)).await.unwrap();
    assert_eq!(
        resp.status(),
        StatusCode::OK,
        "G8-7: vault/info must be 200"
    );
    let body = body_json(resp).await;
    assert!(body.get("user_id").is_some(), "G8-7: must have user_id");
    assert!(
        body.get("api_version").is_some(),
        "G8-7: must have api_version"
    );
    assert!(
        body.get("tier_counts").is_some(),
        "G8-7: must have tier_counts"
    );
    assert!(
...
async fn g8_12_integration_skills_cursor_pagination() {
    ...
    assert_eq!(
        resp1.status(),
        StatusCode::OK,
        "G8-12: skills list must be 200"
    );
    let page1 = body_json(resp1).await;
    let skills1 = page1["skills"]
        .as_array()
        .expect("G8-12: skills must be array");
    assert_eq!(
        skills1.len(),
        2,
        "G8-12: first page must have exactly 2 skills"
    );
    let cursor = page1["next_cursor"]
        .as_str()
        .expect("G8-12: next_cursor must be present when has_more");

    // Fetch next page using the cursor.
    let resp2 = app
        .oneshot(get(
            &format!("/v1/platform/skills?limit=2&after_id={cursor}"),
            TEST_IP,
        ))
        .await
        .unwrap();
    ...
    let skills2 = page2["skills"]
        .as_array()
        .expect("G8-12: skills2 must be array");
    assert!(
        !skills2.is_empty(),
        "G8-12: page 2 must have at least one skill"
    );
...
async fn g8_15_integration_audit_log_written_on_upload() {
    ...
    assert!(
        log_path.exists(),
        "G8-15: audit log file must exist at {}",
        log_path.display()
    );
