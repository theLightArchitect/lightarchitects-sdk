//! Conductor engine — deterministic bulletin board curator.
//!
...
/// Run one conductor cycle (called by scheduler when conductor routine fires).
///
...
pub fn run_cycle(data_dir: &Path, channels: &Channels) -> Result<(), String> {
    ...
    tracing::info!("Conductor cycle starting");
    ...
    tracing::info!("Conductor cycle complete");
    Ok(())
}
...
/// This gives fresh papers at ~7am and ~7pm PST when the conductor runs
...
fn fetch_daily_papers_if_needed(shared_dir: &Path) {
    ...
    // The conductor runs in a sync context (called from scheduler tick).
...
/// since the conductor runs in a sync context.
...
///
/// Runs every conductor cycle but operations are idempotent and cheap
