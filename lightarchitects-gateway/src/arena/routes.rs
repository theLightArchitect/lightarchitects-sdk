...
pub async fn rest_action(
    ...
    // Reassemble the request to extract the JSON body via axum's extractor.
    let reassembled = axum::http::Request::from_parts(parts, body);
    let Json(json_body): Json<Value> = match Json::from_request(reassembled, &()).await {
        Ok(v) => v,
        Err(_) => {
            return rest_error(
                StatusCode::BAD_REQUEST,
...
pub async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if state.config.siblings.l_arc.is_none() {
        siblings
            .entry("l-arc".to_owned())
...
pub async fn larc_chat(
    ...
    let (Some(url), Some(token)) = (&state.config.larc_endpoint_url, &state.config.larc_hf_token)
    else {
        return rest_error(
            StatusCode::SERVICE_UNAVAILABLE,
            "larc_not_configured",
            "L-ARC endpoint not configured (set LARC_ENDPOINT_URL and LARC_HF_TOKEN)",
        );
    };
