...
use crate::engine::{ToolCallRecord, Trace};
use crate::exercises::{Exercise, ExpectedToolCall};
...
fn score_hallucination(
    actual: &[ToolCallRecord],
    available: &[lightarchitects_core::action::ToolInfo],
) -> (f64, String) {
    if actual.is_empty() {
...
mod tests {
    use super::*;
    use crate::engine::ToolCallResult;
    ...
    fn hallucination_detects_fake_tool() {
        ...
        let available = vec![lightarchitects_core::action::ToolInfo {
            name: "real_tool".into(),
            description: None,
