//! Adversarial integration tests for `lightarchitects-core`.
//!
...
use lightarchitects_core::constants::{MAX_CONTENT_LENGTH_HEADERS, MAX_RESPONSE_BYTES};
use lightarchitects_core::error::{SdkError, TransportError};
use lightarchitects_core::sibling::SiblingId;
use lightarchitects_core::transport::StdioTransport;
...
//
// All `serde_json::from_str` / `from_value` call sites in `lightarchitects-core` that
