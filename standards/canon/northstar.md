...
11. **Post-Build Guarantees Contract verification** (Canon XLV, added 2026-06-02) — at close-out, the cold-context independent runner executes the plan's declared `verification_script` (`scripts/post-build-verify-<codename>.sh`) and grades against Tier 1 HARD guarantees. Build does not promote to `status: promoted` until all HARD guarantees PASS or the operator explicitly waives a specific G-row with rationale logged to `audit/guarantee-waivers-<date>.log`. The Contract is complementary to the LDB §7.7 deliverable_benchmark — LDB grades quality on a continuum, the Contract grades claims on binary pass/fail. Both must satisfy promotion gates.
...
| **§Q Copilot-Supervised Orchestration** | 🔄 Partial | build.northstar_text capture ✅ (SQLite + SOUL helix); northstar injection ✅ (set_northstar_text); per-wave evaluation ✅ (evaluate_wave, neutral stub); SSE supervisor stream ✅ (GET /supervisor/events); drift detection ✅ (configurable-N SupervisorState); next-step proposal cards ✅ (ProposalCard.svelte); supervisor state API ✅ (GET /supervisor/state); action routing ✅ (SupervisorRoutingConfig); HITL escalation ✅ (escalate_to_hitl); BuildDetail.svelte wiring 🔄 (blocked — feat/helix-viz-remap FILE_CLAIM) |
