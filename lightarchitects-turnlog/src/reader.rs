...
use crate::chain::{GenesisBlock, derive_session_key, pepper_fingerprint, verify_chain};
...
impl TurnLogReader {
    ...
    async fn summarize_session(&self, session_id: &str, is_active: bool) -> Option<SessionSummary> {
        ...
        let paused_weight = entries
            .iter()
            .rev()
            .find(|e| e.kind() == crate::entry::EntryKind::SessionPaused)
            .map(|e| {
                let raw = e
                    .span
                    .metadata
...
mod tests {
    ...
    use super::*;
    use crate::writer::{EndReason, TurnLogWriter};
