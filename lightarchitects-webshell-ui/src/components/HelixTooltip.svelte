...
{#if node}
  ...
  >
    <div class="bg-[var(--la-drawer-bg)]/95 border border-[var(--la-focus-ring)]/30 rounded-lg px-3 py-2 shadow-[0_0_12px_rgba(255,215,0,0.15)] backdrop-blur-sm">
      <!-- Sibling badge + path -->
      <div class="flex items-center gap-2 mb-1">
        ...
        <span class="text-[9px] text-[var(--la-text-dim)]">·</span>
        <span class="text-[9px] text-[var(--la-text-label)]">★ {node.significance.toFixed(1)}</span>
      </div>
      <!-- Path -->
      <div class="text-[10px] text-[var(--la-text-bright)] font-mono truncate mb-1">{node.path}</div>
      <!-- Excerpt -->
      {#if node.excerpt}
        <div class="text-[9px] text-[var(--la-text-dim)] line-clamp-2 leading-relaxed">{node.excerpt}</div>
      {/if}
    </div>
  </div>
{/if}
